  ...
  module.exports = {
    async addToFavorites(req, res, next) {
      try {
        ...
        return res
          .status(202)
          .send({ msg: "Article successfully added to favorites." });
      } catch (error) {
        return res
          .status(400)
          .send({ msg: "User with provided ID do not exist." });
      }
    },
    async removeFromFavorites(req, res, next) {
      try {
        ...
        return res
          .status(202)
          .send({ msg: "Article successfully removed from favorites." });
      } catch (error) {
        return res
          .status(400)
          .send({ msg: "User with provided ID do not exist." });
    ...
    async getBasket(req, res, next) {
      try {
        ...
        const totalAmount = user.basket.reduce(
    ...
    async addToBasket(req, res, next) {
      try {
        ...
        return res
          .status(202)
          .send({ msg: "Article successfully added to basket." });
      } catch (error) {
        return res
          .status(400)
          .send({ msg: "User with provided ID do not exist." });
    ...
    async removeFromBasket(req, res, next) {
      try {
        ...
        await User.findOneAndUpdate(
          { _id: userId },
          { $pull: { basket: articleId } }
        );
        return res
          .status(202)
          .send({ msg: "Article successfully removed from basket." });
      } catch (error) {
        return res
          .status(400)
          .send({ msg: "User with provided ID do not exist." });
    ...
    async placeOrder(req, res, next) {
      try {
        ...
        const order = new Order({ creator, articles: basket, totalAmount });
        order.save();
    ...
    async getUsers(req, res, next) {
      try {
        ...
        return res
          .status(507)
          .send({ msg: "The method could not be performed." });
      }
    },
    async updateUser(req, res, next) {
      try {
        ...
        if (password !== "") {
          ...
          return res
            .status(202)
            .send({ msg: "Successfully updated user data." });
        }
        await User.findByIdAndUpdate(userId, {
          username,
          admin: admin === "true" ? true : false,
