...
module.exports = {
  ...
  async getOrders(req, res, next) {
    ...
    if (!orders) { return res.status(507).send('The method could not be performed on the resource because the server is unable to store the representation needed to successfully complete the request.') }
    res.send(orders);
  },
  ...
  async getHistoryOrders(req, res, next) {
    ...
    if (!historyOrders) { return res.status(507).send('The method could not be performed on the resource because the server is unable to store the representation needed to successfully complete the request.') }
